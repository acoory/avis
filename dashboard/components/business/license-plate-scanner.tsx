...
export function LicensePlateScanner({ country, onClose, onConfirm }: LicensePlateScannerProps) {
  ...
  const processImage = useCallback(
    async (image: HTMLCanvasElement | File) => {
      ...
      if (!worker || isScanningRef.current) {
        return null;
      }
      ...
      try {
        ...
        const previousCandidate = stableCandidateRef.current;
        const stableCount =
          previousCandidate?.value === normalizedValue ? previousCandidate.count + 1 : 1;
        ...
        const detectedCountry = detectLicensePlateCountry(normalizedValue, country);
        const detectedResult = {
          value: candidate.raw || normalizedValue,
  ...
  const captureScanArea = useCallback(() => {
    ...
    const sourceHeight = Math.min(sourceWidth / 3.8, video.videoHeight * 0.35);
    const sourceX = (video.videoWidth - sourceWidth) / 2;
    const sourceY = (video.videoHeight - sourceHeight) / 2;
    const outputWidth = 1280;
    const outputHeight = Math.round(outputWidth / 3.8);
  ...
  const scheduleAutomaticScan = useCallback(() => {
    ...
    const runScan = async () => {
      if (!isMountedRef.current || result || cameraStatus !== "active" || !workerRef.current) {
        return;
      }

      const scanArea = captureScanArea();
      if (scanArea) {
        await processImage(scanArea);
      }

      if (isMountedRef.current && !stableCandidateRef.current?.count) {
        setProgress(0);
      }
    ...
    scanTimerRef.current = window.setTimeout(runScan, 600);
  }, [
    cameraStatus,
    captureScanArea,
    processImage,
  ...
  useEffect(() => {
    ...
    async function loadOcrModel() {
      try {
        ...
        setIsModelLoading(false);
        toast.error("Le moteur de reconnaissance n'a pas pu demarrer.");
      }
    }
  ...
  useEffect(() => {
    if (cameraStatus === "active" && !isModelLoading && !result) {
      scheduleAutomaticScan();
    }

    return stopAutomaticScan;
  }, [cameraStatus, isModelLoading, result, scheduleAutomaticScan, stopAutomaticScan]);
  ...
  async function handlePhoto(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      const firstResult = await processImage(file);
      if (!firstResult) {
        stableCandidateRef.current = null;
        const worker = workerRef.current;
        if (!worker) {
          toast.error("Le moteur OCR n'est pas encore pret.");
          return;
        }
        ...
        const candidate = extractLicensePlateCandidate(recognition.data.text, country);
        if (!candidate) {
          toast.error("Aucune plaque exploitable detectee.");
          return;
        }

        setResult({
          value: candidate.raw || candidate.normalized,
          country: detectLicensePlateCountry(candidate.normalized, country),
          confidence: Math.max(0, Math.min(100, Math.round(recognition.data.confidence))),
        });
      }
    } catch {
  ...
  const scannerReady = cameraStatus === "active" && !isModelLoading;
      ...
      <div className="max-h-[100dvh] w-full overflow-y-auto rounded-t-lg bg-white shadow-xl md:max-h-[92dvh] md:max-w-lg md:rounded-lg">
        <div className="sticky top-0 z-10 flex items-start justify-between border-b border-gray-200 bg-white p-4">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500">
              Place la plaque dans le cadre, la detection est automatique.
            </p>
          </div>
        ...
        <div className="space-y-4 p-4">
          <div className="relative aspect-[4/3] overflow-hidden rounded-md bg-gray-950">
            ...
            <div
              className={[
                "pointer-events-none absolute left-[5%] top-1/2 aspect-[3.8/1] w-[90%] -translate-y-1/2 rounded-md border-2",
                ...
                <div>
                  ...
                  <p className="mt-3 text-sm font-medium">
                    {cameraStatus === "unavailable"
                      ...
                      : "Demarrage de la camera et du moteur OCR"}
                  </p>
                  {isModelLoading ? (
                    <p className="mt-1 text-xs text-gray-300">Chargement : {progress}%</p>
                  ) : null}
                </div>
              </div>
            ) : null}
          ...
          {result ? (
            <div className="rounded-md border border-emerald-200 bg-emerald-50 p-4">
              <div className="flex items-center gap-2 text-emerald-800">
                ...
                <p className="text-sm font-medium">Plaque detectee automatiquement</p>
              </div>
